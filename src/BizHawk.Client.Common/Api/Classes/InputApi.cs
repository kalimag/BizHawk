...
namespace BizHawk.Client.Common
{
	public sealed class InputApi : IInputApi
	{
		...
		public Dictionary<string, bool> Get()
		{
			...
			foreach (var (button, _) in _inputManager.ApiInputCoalescer.BoolButtons().Where(kvp => kvp.Value)) buttons[button] = true;
			return buttons;
		}
		...
		public IReadOnlyList<string> GetPressedButtons()
			=> _inputManager.ApiInputCoalescer.BoolButtons().Where(static kvp => kvp.Value)
